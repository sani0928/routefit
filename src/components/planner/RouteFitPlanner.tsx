...
export function RouteFitPlanner() {
  const { places, setPlaces, returnToStart, setReturnToStart, fixedVisitOrders, setFixedVisitOrders, result, setResult, resultSnapshot, setResultSnapshot, routeNeedsRecalculation, setRouteNeedsRecalculation, status, setStatus, hoveredSegmentIndex, setHoveredSegmentIndex, selectedSegmentIndex, setSelectedSegmentIndex } = useRoutePlanner();
  const { isSharingRoute, setIsSharingRoute, shareDialogUrl, setShareDialogUrl } = useRouteSharing();
  ...
  const mapListPlaces = useMemo(() => {
    if (!listManagerOpen) return undefined;
    if (activeList) return savedListPlaces.map((place) => ({ ...place, color: activeList.color }));
    return member.placeLists.flatMap((list) => (savedPlacesByListId[list.id] ?? []).map((place) => ({ ...place, color: list.color })));
  }, [activeList, listManagerOpen, member.placeLists, savedListPlaces, savedPlacesByListId]);
  ...
  useEffect(() => {
    ...
    void fetch("/api/place-lists/matches", {
      ...
      .then((matches) => {
        ...
        const requestedProviderIds = new Set(matchPlaces.flatMap((place) => place.providerId ? [place.providerId] : []));
        setSavedListIdsByProviderId((current) => {
          const next = { ...current };
          requestedProviderIds.forEach((providerId) => {
            const listIds = nextMatchesByProviderId[providerId] ?? [];
            if (listIds.length > 0) next[providerId] = listIds;
  ...
  useEffect(() => () => quickSearchMatchesAbortRef.current?.abort(), []);

  const refreshQuickSearchMembership = useCallback((quickResults: PlaceSearchResult[]) => {
    quickSearchMatchesAbortRef.current?.abort();
    if (!memberStateReady || !member.authenticated || !workspaceRestored) return;

    const matchPlaces = quickResults
      .filter((place): place is PlaceSearchResult & { providerId: string } => Boolean(place.providerId))
      .slice(0, 15)
      .map(({ providerId, name, latitude, longitude }) => ({ id: `quick:${providerId}`, providerId, name, latitude, longitude }));
    if (matchPlaces.length === 0) return;

    const controller = new AbortController();
    quickSearchMatchesAbortRef.current = controller;
    void fetch("/api/place-lists/matches", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ places: matchPlaces }),
      ...
      .then((matches) => {
        if (controller.signal.aborted) return;
        const matchedListIdsByProviderId = Object.values(matches).reduce<Record<string, string[]>>((current, match) => {
          current[match.providerId] = [...new Set([...(current[match.providerId] ?? []), ...match.listIds])];
          return current;
        }, {});
        setSavedListIdsByProviderId((current) => {
          const next = { ...current };
          matchPlaces.forEach(({ providerId }) => {
            const listIds = matchedListIdsByProviderId[providerId] ?? [];
            if (listIds.length > 0) next[providerId] = listIds;
            else delete next[providerId];
          });
          return next;
  ...
  }, [memberStateReady, member.authenticated, workspaceRestored]);
  useEffect(() => {
    if (!memberStateReady || !member.authenticated || !workspaceRestored) return;
    const workspace = { returnToStart, places, fixedVisitOrders };
  ...
  useEffect(() => {
    if (!listManagerOpen || selectedListId || !member.authenticated) return;
    const missingListIds = member.placeLists
      .filter((list) => !Object.prototype.hasOwnProperty.call(savedPlacesByListId, list.id))
      .map((list) => list.id);
    ...
    void Promise.all(missingListIds.map(async (listId) => {
      ...
      if (response.status === 404) return { listId, places: [], missing: true };
      if (!response.ok) throw new Error("saved-place-list-load-failed");
      const places = (await response.json() as { places: SavedPlace[] }).places;
      return { listId, places, missing: false };
    })).then((entries) => {
      if (cancelled) return;
      const missingListIdSet = new Set(entries.filter((entry) => entry.missing).map((entry) => entry.listId));
      setSavedPlacesByListId((current) => ({ ...current, ...Object.fromEntries(entries.map(({ listId, places }) => [listId, places])) }));
      if (missingListIdSet.size > 0) {
        setMember((current) => ({ ...current, placeLists: current.placeLists.filter((list) => !missingListIdSet.has(list.id)) }));
      }
    }).catch(() => {
      if (!cancelled) notify.error("장소 리스트를 불러오지 못했습니다.");
    });
    return () => { cancelled = true; };
  }, [listManagerOpen, member.authenticated, member.placeLists, savedPlacesByListId, selectedListId]);
  useEffect(() => {
  ...
  async function savePlace(selectedListIds: string[], initiallySelectedListIds: string[]) {
    ...
    const removeOutcomes = await Promise.all(listIdsToRemove.map(async (listId) => {
      ...
      try {
        ...
        if (!response.ok) throw new Error("저장한 장소��삭제하지 못했습니다.");
        return "removed";
      } catch (error) {

...
import { Check, ChevronRight, GripVertical, Hand, Info, LockKeyhole, MapPin, Route, Search, Trash2, Undo2 } from "lucide-react";

type GuideStage = "places" | "calculating" | "result";
type GuidePlace = { id: string; name: string; position: { x: number; y: number } };
type GuideFaqMessage = { id: string; role: "question" | "answer"; content: string };
...
export function InteractiveGuide() {
  ...
  const selectFaq = (item: (typeof guideFaqItems)[number]) => {
    ...
    const answer: GuideFaqMessage = { id: `${id}-answer`, role: "answer", content: item.answer };
    const showAnswer = () => {
      setIsFaqTyping(true);
  ...
  useEffect(() => () => { if (faqTypingTimeoutRef.current) clearTimeout(faqTypingTimeoutRef.current); if (faqExitTimeoutRef.current) clearTimeout(faqExitTimeoutRef.current); }, []);
  useEffect(() => {
  ...
  return <main className="guide-page">
    ...
    <section className="guide-step guide-step-tips" id="guide-tips" aria-labelledby="gesture-title"><div className="guide-gesture"><div className="guide-gesture-copy"><span className="guide-kicker">간단한 조작 팁</span><h2 id="gesture-title">카드를 가로로 밀어보세요.</h2><p>모바일에서는 카드의 빠른 기능으로 머무는 시간을 조절할 수 있어요.</p><button type="button" onClick={() => setSwipeOffset((current) => current === 0 ? -getGestureRevealWidth() : 0)}>{swipeOffset === 0 ? "동작 보기" : "카드 닫기"}<ChevronRight /></button></div><div className="guide-swipe-demo"><div className="guide-swipe-stage"><div className="guide-swipe-actions"><button type="button" aria-label="장소 삭제"><Trash2 /></button><button type="button" aria-label="방문 순서 보장"><LockKeyhole /></button></div><div className="guide-swipe-card" style={{ transform:`translateX(${swipeOffset}px)` }} onPointerDown={handleGestureStart} onPointerMove={handleGestureMove} onPointerUp={handleGestureEnd} onPointerCancel={() => setSwipeOffset(0)}><span className="guide-swipe-grip"><GripVertical /></span><b>2</b><span><strong>방문 장소</strong><small>가로로 밀면 빠른 기능 확인</small></span><span className="guide-swipe-hand"><Hand /></span></div></div><small className="guide-swipe-hint"><Info /> 가로로 밀면 빠른 기능이 나타납니다.</small></div></div><div className="guide-use-cases"><div><span className="guide-kicker">여러 장소를 방문하는 모든 날</span><h2>외근과 약속 사이, 이동 순서를<br />더 가볍게 정해 보세요.</h2><p>RouteFit은 여러 방문 장소를 한 번에 정리하고 이동 경로를 확인할 수 있어,<br /> 목적이 다른 하루 일정에도 유용합니다.</p></div><ul><li><strong>영업 · 외근 방문</strong><span>거래처 미팅 장소, 업무 거점을 한 번에 추가해 방문 순서를 확인하세요.</span></li><li><strong>배송 · 현장 업무</strong><span>여러 배송지와 작업 현장을 모아 이동 동선을 빠르게 정리할 수 있습니다.</span></li><li><strong>여행 일정</strong><span>관광지, 식당, 숙소처럼 하루에 들를 곳이 많은 여행 계획에 사용해 보세요.</span></li><li><strong>데이트 · 약속 코스</strong><span>카페, 전시, 식사처럼 여러 약속 장소를 자연스러운 순서로 이어 보세요.</span></li></ul></div></section>

    <section className="guide-step guide-step-faq" id="guide-faq" aria-labelledby="guide-faq-title"><div className="guide-faq"><div className="guide-faq-heading"><span className="guide-kicker">자주 묻는 질문</span><h2 id="guide-faq-title">궁금한 점을 물어보세요.</h2><p>이동 동선부터 회원 혜택까지, 자주 묻는 질문을 모아뒀어요.</p></div><div className="guide-faq-messenger"><div className="guide-faq-chat" aria-live="polite">{faqMessages.map((message) => <p key={message.id} className={`guide-faq-message is-${message.role}${faqLeavingIds.includes(message.id) ? " is-leaving" : ""}`}><span>{message.content}</span></p>)}{isFaqTyping && <p className="guide-faq-typing"><span /><span /><span /></p>}</div><div className="guide-faq-questions">{guideFaqItems.map((item) => <button type="button" key={item.id} onClick={() => selectFaq(item)}>{item.question}</button>)}</div></div></div><div className="guide-final-cta"><div><h2>이제 시작해 볼까요?</h2><p>장소를 추가하면 RouteFit이 동선을 빠르게 계산해드려요.</p></div><a href="/">RouteFit 시작하기</a></div><footer className="guide-contact-footer">문의 및 버그 제보 : <a href="mailto:kksan12@gmail.com">kksan12@gmail.com</a></footer></section>

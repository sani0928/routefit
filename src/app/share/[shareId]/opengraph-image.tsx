...
export default async function OpenGraphImage({ params }: Props) {
  ...
  return new ImageResponse(
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between", padding: "72px", color: "white", background: "linear-gradient(120deg, #173b7d 0%, #2563eb 44%, #139bdf 73%, #1bb9a8 100%)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "16px", fontSize: "34px", fontWeight: 800 }}><span style={{ width: "42px", height: "42px", borderRadius: "50%", background: "linear-gradient(135deg, #67e8f9, #5eead4)" }} />RouteFit</div>
      <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
        <div style={{ fontSize: "29px", opacity: .8 }}>공유된 방문 동선</div>
        <div style={{ fontSize: "76px", fontWeight: 800, letterSpacing: "-4px" }}>{summary ? formatTime(totalDuration) : "공유 기간 종료"}</div>
      </div>
